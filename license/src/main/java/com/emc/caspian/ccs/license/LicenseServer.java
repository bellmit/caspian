...
import com.emc.caspian.ccs.license.util.GetKeystoneDetails;
...
public class LicenseServer {
	...
	private static Server createJettyServer(Application application) {
		...
		try{
			keystoneUrl = GetKeystoneDetails.getKeystoneBaseUrl();
			AppLogger.info("Keystoneurl: "+keystoneUrl);
		}catch(Exception e){
			AppLogger.logException(e);
			AppLogger.error("License service is exiting");
			System.exit(1);
		}
		...
		char[] ksPwd=null;
		try{
			ksPwd = GetKeystoneDetails.getKeystonePassword();
		}catch(Exception e){
			AppLogger.logException(e);
			AppLogger.error("License service is exiting");
			System.exit(1);
		}
		resourceConfig.registerInstances(new KeystoneJerseyAuthFilter(keystoneUrl,"csa",String.valueOf(ksPwd)));
